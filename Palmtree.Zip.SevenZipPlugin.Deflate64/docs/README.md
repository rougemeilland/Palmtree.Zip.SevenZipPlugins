# Palmtree.IO.Compression.Stream.Plugin.SevenZip.Deflate64

## 1. Overview

<!--
このパッケージは、パッケージ `Palmtree.IO.Compression.Archive.Zip`で Deflate64 圧縮方式をサポートするためのプラグインライブラリのパッケージです。
-->
This package is a plugin library package to support the Deflate64 compression method in the package `Palmtree.IO.Compression.Archive.Zip`.
...
<!--
Deflate64 圧縮方式を有効にするためには、アプリケーションプログラムで最初に以下のステートメントを実行してください。
-->
To enable the Deflate64 compression method, first execute the following statement in your application program.

```csharp
 Palmtree.IO.Compression.Stream.Plugin.SevenZip.Deflate64CoderPlugin.EnablePlugin();
```
